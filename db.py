...
""")
    ...
    defaults = {
        ...
        "max_concurrent": "8",
        "trading_window": "continuously",  # ✅ FIX: Default to continuous trading
    }
...
def open_trade(epoch: int, symbol: str, side: str, signal: str, confidence: int,
               composite: float, entry_price: float, qty: float, leverage: int,
               sl: float, tp: float, order_id: str, tag: str, mode: str,
               components: Dict, vol_score: float = 0.0, range_pct: float = 0.0) -> int:
    notional = qty * entry_price
    c = _conn()
    cur = c.execute("""INSERT INTO trades
        (ts_open,epoch,symbol,side,signal,confidence,composite,entry_price,qty,leverage,
        notional,sl_price,tp_price,outcome,order_id,tag,mode,components,vol_score,range_pct)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'OPEN',?,?,?,?,?,?)""",
        (int(time.time()), epoch, symbol, side, signal, confidence, composite,
        entry_price, qty, leverage, notional, sl, tp, order_id, tag, mode,
        json.dumps(components), vol_score, range_pct))
    tid = cur.lastrowid
    c.execute("""INSERT OR REPLACE INTO open_positions
        (symbol,epoch,side,signal,trade_id,order_id,entry_price,qty,leverage,notional,
        sl_price,tp_price,ts_open,confidence) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (symbol, epoch, side, signal, tid, order_id, entry_price, qty, leverage,
        notional, sl, tp, int(time.time()), confidence))
    c.commit(); c.close()
    return tid
...
def close_trade(trade_id: int, exit_price: float, pnl: float, initial_cap: float):
    ...
    c.execute("""UPDATE trades SET ts_close=?,exit_price=?,pnl_usdt=?,pnl_pct=?,
        outcome=?,duration_s=? WHERE id=?""", (now, exit_price, pnl, pct, out, dur, trade_id))
    if row:
...
def _upd_sym(c, symbol: str, outcome: str, pnl: float, dur: float):
    ...
    if r:
        c.execute("""UPDATE symbol_stats SET wins=?,losses=?,total_pnl=?,learned_bias=?,
            sl_mult=?,tp_mult=?,avg_dur_s=?,last_updated=? WHERE symbol=?""",
            (wins, losses, old_p + pnl, new_bias, round(sl_m,2), round(tp_m,2),
             round(avg_dur), int(time.time()), symbol))
    else:
        c.execute("""INSERT INTO symbol_stats
...
def get_epoch_trades(epoch_num: int) -> List[Dict]:
    c = _conn()
    rows = c.execute("SELECT * FROM trades WHERE epoch=? ORDER BY ts_open", (epoch_num,)).fetchall()
    c.close()
    return [dict(r) for r in rows]
...
def log_capital(epoch: int, day_in_epoch: int, balance: float, target_now: float,
                target_eod: float, ahead_pct: float, mode: str, open_pos: int):
    c = _conn()
    c.execute("""INSERT INTO capital_log
        (ts,epoch,day_in_epoch,balance,target_now,target_eod,ahead_pct,mode,open_pos)
        VALUES (?,?,?,?,?,?,?,?,?)""",
        (int(time.time()), epoch, day_in_epoch, balance, target_now, target_eod,
         ahead_pct, mode, open_pos))
    c.commit(); c.close()

def get_capital_log(limit: int = 300) -> List[Dict]:
    c = _conn()
    rows = c.execute("SELECT * FROM capital_log ORDER BY ts DESC LIMIT ?", (limit,)).fetchall()
    c.close()
...
def close_epoch_record(epoch_num: int, end_bal: float):
    ...
    trades = c.execute("SELECT * FROM trades WHERE epoch=? AND outcome!='OPEN'", (epoch_num,)).fetchall()
    ...
    c.execute("""UPDATE epochs SET end_ts=?,end_bal=?,target_met=?,pnl_usdt=?,
        pct_gain=?,total_trades=?,wins=?,losses=?,win_rate=? WHERE epoch_num=?""",
        (int(time.time()), end_bal, met, pnl, pct, len(trades), wins, len(trades)-wins,
...
def snap_hour(epoch: int, balance: float, hourly_pnl: float, trades: int, wins: int,
              open_pos: int, mode: str):
    label = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:00")
    wr = round(wins/trades*100,1) if trades > 0 else 0.0
    c = _conn()
    c.execute("""INSERT INTO hourly_snaps
...
def get_hourly_snaps(limit: int = 96) -> List[Dict]:
    c = _conn()
    rows = c.execute("SELECT * FROM hourly_snaps ORDER BY ts DESC LIMIT ?", (limit,)).fetchall()
    c.close()
    return [dict(r) for r in reversed(rows)]
...
# ✅ NEW: log_trade function for direct trade logging (used by engine.py)
def log_trade(data: Dict) -> int:
    """
    Log a completed trade directly.
    ...
    cur = c.execute("""INSERT INTO trades
        ...
        (
            ...
            data["entry_price"],
            data.get("exit_price", data["entry_price"] * (1 + data.get("pnl_usdt", 0)/max(data["entry_price"],1))),
            data["qty"],
