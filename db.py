...
""")
    ...
    try:
        ...
        pass
    for col in ["tier1_done INTEGER DEFAULT 0", "tier2_done INTEGER DEFAULT 0",
                "tier3_done INTEGER DEFAULT 0", "breakeven_set INTEGER DEFAULT 0"]:
        try:
            c.execute(f"ALTER TABLE open_positions ADD COLUMN {col}")
            c.commit()
        except Exception:
            pass
...
def open_trade(
    ...
    c.execute(
        ...
        leverage,notional,sl_price,tp_price,ts_open,confidence,partial_closed,tier1_done,tier2_done,tier3_done,breakeven_set)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,0,0,0,0)""",
