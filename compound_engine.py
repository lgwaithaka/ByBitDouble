...
AGGRESSION MODES
  CONSERVATIVE  — behind target by >15%: protect capital, higher conf floor
...
# Aggression mode thresholds (ahead_pct = actual / target - 1)
AGGR_BEHIND_HARD  = -0.20   # more than 20% behind → conservative
AGGR_BEHIND_SOFT  = -0.08   # 8-20% behind          → protect
AGGR_AHEAD_PUSH   =  0.10   # 10%+ ahead             → aggressive
AGGR_AHEAD_TURBO  =  0.28   # 28%+ ahead             → turbo
...
class EpochState:
    ...
    # Mode
    mode:            str  = "NORMAL"   # CONSERVATIVE | NORMAL | AGGRESSIVE | TURBO
    mode_since:      int  = 0
...
class CompoundEngine:
    ...
    def epoch_progress_pct(self, balance: float) -> float:
        ...
        s = self.state
        return (balance - s.epoch_start_bal) / (s.epoch_target - s.epoch_start_bal) * 100
    ...
    def _update_mode(self, balance: float):
        ...
        if s.epoch_cb_active or s.daily_cb_active:
            ...
            new_mode = "CONSERVATIVE"
        elif ahead <= AGGR_BEHIND_HARD:
            new_mode = "CONSERVATIVE"
        elif ahead <= AGGR_BEHIND_SOFT:
            new_mode = "NORMAL"          # protect — don't gamble to catch up
        elif ahead >= AGGR_AHEAD_TURBO and s.streak >= 3:
            new_mode = "TURBO"
        elif ahead >= AGGR_AHEAD_PUSH:
            new_mode = "AGGRESSIVE"
        else:
            new_mode = "NORMAL"

        if new_mode != s.mode:
            logger.info(f"Mode change: {s.mode} → {new_mode} (ahead={ahead:+.1%})")
            s.mode       = new_mode
    ...
    def compute_risk_pct(self) -> float:
        ...
        if s.streak >= 5:     streak_mult = 1.25
        elif s.streak >= 3:   streak_mult = 1.12
        elif s.streak >= 1:   streak_mult = 1.00
        elif s.streak <= -3:  streak_mult = 0.60
        elif s.streak <= -1:  streak_mult = 0.80
    ...
    def compute_confidence_floor(self) -> int:
        """Confidence threshold — lower in aggressive modes, higher in conservative."""
        floors = {"CONSERVATIVE": 55, "NORMAL": 44, "AGGRESSIVE": 38, "TURBO": 34}
        return floors.get(self.state.mode, 44)

    def compute_max_concurrent(self) -> int:
        limits = {"CONSERVATIVE": 3, "NORMAL": 6, "AGGRESSIVE": 10, "TURBO": 14}
        return limits.get(self.state.mode, 6)
    ...
    def compute_sl_tp_mults(self, symbol_sl: float = 1.2, symbol_tp: float = 2.4) -> Tuple[float, float]:
        ...
        s = self.state
        mode_adj = {
            "CONSERVATIVE": (1.0,  2.0),  # tighter TP to lock gains
            "NORMAL":        (1.0,  1.0),
            "AGGRESSIVE":    (1.0,  1.15),
            "TURBO":         (0.95, 1.30),
        }.get(s.mode, (1.0, 1.0))
