...
class BybitClient:
    def __init__(self, api_key: str, api_secret: str, testnet: bool = False):
        ...
        # Log key prefix for debugging (safe — only first 6 chars)
        logger.info(f"BybitClient init | base={self.base} | key={api_key[:6]}*** | secret={'set' if api_secret else 'EMPTY'}")
    ...
    async def _get(self, path, params=None, auth=True):
        ...
        try:
            async with sess.get(f"{self.base}{path}", params=params, headers=hdrs) as r:
                # Read raw text first to diagnose non-JSON responses
                raw_text = await r.text()
                try:
                    data = json.loads(raw_text)
                except json.JSONDecodeError as je:
                    logger.error(f"GET {path} | JSON parse failed: {je}")
                    logger.error(f"GET {path} | HTTP status: {r.status}")
                    logger.error(f"GET {path} | Raw response (first 500 chars): {raw_text[:500]}")
                    return {"retCode": -1, "result": {}}
                # Sanitize: ensure "result" is never None
                if data.get("result") is None:
                    data["result"] = {}
                # Sanitize nested: ensure "list" inside result is never None
                if isinstance(data.get("result"), dict) and data["result"].get("list") is None:
                    data["result"]["list"] = []
        ...
        try:
            async with sess.post(f"{self.base}{path}", data=bs, headers=hdrs) as r:
                ...
                try:
                    ...
                    logger.error(f"POST {path} | JSON parse failed: {je}")
                    logger.error(f"POST {path} | HTTP status: {r.status}")
                    logger.error(f"POST {path} | Raw response (first 500 chars): {raw_text[:500]}")
                    return {"retCode": -1, "result": {}}
                # Sanitize: ensure "result" is never None
                if data.get("result") is None:
                    data["result"] = {}
