...
class QuantSignalEngine:
    ...
    def analyze(self, sym, k3m, k5m, k15m, k1h, k4h,
        ...
        if k4h is not None and len(k4h) >= 50:
            ...
            # ADX < 18 = ranging, no directional trades
            if adx_4h_val < 18:
                hold["veto_reason"] = f"RANGING_ADX4H={adx_4h_val:.1f}<18"
                return hold
            ...
            if adx_1h_tmp < 20:
                hold["veto_reason"] = f"RANGING_ADX1H={adx_1h_tmp:.1f}<20"
                return hold
        ...
        # STRICT thresholds — tuned from trade analysis
        # Raised NORMAL conf from 65→68, AGGRESSIVE from 58→62
        # to filter out marginal trades that historically lost
        thresholds  = {"CONSERVATIVE":0.45,"NORMAL":0.38,"AGGRESSIVE":0.30,"TURBO":0.25}
        conf_floors = {"CONSERVATIVE":72,  "NORMAL":68,  "AGGRESSIVE":62,  "TURBO":55}
