...
class QuantSignalEngine:
    ...
    def analyze(self, sym, k3m, k5m, k15m, k1h, k4h,
                ...
                learned_bias=0.0, mode="NORMAL", **kwargs):
        ...
        # ── BTC Macro Confirmation ───────────────────────────────────────────────
        # Do not enter altcoin LONGs when BTC is in a 4H downtrend.
        # BTC 4H klines passed as btck4h kwarg when available.
        btc_macro = 0  # 0=unknown, 1=bull, -1=bear
        if 'BTCUSDT' not in sym:  # Only check for non-BTC symbols
            btck = kwargs.get('btck4h')
            if btck is not None and len(btck) >= 50:
                btc_c = btck[:,4]
                btc_e21 = ema(btc_c,21)
                btc_st  = calc_supertrend(btck,10,3.0)
                btc_macro = 1 if float(btc_c[-1]) > btc_e21[-1] and btc_st==1 else -1
        ...
        if k4h is not None and len(k4h) >= 50:
            ...
            # ADX < 15 = truly ranging, no directional trades
            if adx_4h_val < 15:
        ...
        # Volume confirmation GATE (hard requirement): entry candle volume > 1.3x average
        vol_gate_pass = True
        if k5m is not None and len(k5m) >= 22:
            try:
                vols = k5m[-21:,5]
        ...
        # RSI divergence bonus: +15 confidence when divergence detected
        div_bonus = 0
        if k5m is not None and len(k5m) >= 20:
            try:
                ...
                rsi_recent = [rsi(c5d[:i],14) for i in range(len(c5d)-5,len(c5d)+1)]
                if macro==1 and c5d[-1]<c5d[-5] and rsi_recent[-1]>rsi_recent[0]:
                    div_bonus = 15  # Bullish divergence
                elif macro==-1 and c5d[-1]>c5d[-5] and rsi_recent[-1]<rsi_recent[0]:
                    div_bonus = 15  # Bearish divergence
                if div_bonus:
                    confidence = min(100, confidence + div_bonus)
                    logger.debug(f"{sym}: RSI divergence detected +{div_bonus} conf")
            except Exception:
                pass

        if macro==1 and comp>=thresh and confidence>=conf_floor and vol_gate_pass:
            signal="LONG"
        elif macro==-1 and comp<=-thresh and confidence>=conf_floor and vol_gate_pass:
            signal="SHORT"
        else:
            signal="HOLD"
            if not vol_gate_pass:
                hold["veto_reason"] = f"VOLUME_GATE_FAIL: entry volume < 1.3x average"
            elif btc_penalized and confidence < conf_floor:
                hold["veto_reason"] = f"BTC_MACRO_BEARISH: conf {confidence} < raised floor {conf_floor}"
