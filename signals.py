...
class QuantSignalEngine:
    ...
    def analyze(self, sym, k3m, k5m, k15m, k1h, k4h,
        ...
        if k5m is not None and len(k5m) >= 55:
            ...
            scores["st_5m"]  = 0.7 if st5==macro else -0.5
            scores["macd_5m"] = 0.6 if (hist5>0 and macro==1) or (hist5<0 and macro==-1) else -0.3
        ...
        if k3m is not None and len(k3m)>=30:
            ...
            if macro==1:
                scores["cross_3m"] = 0.8 if cross_up else (0.3 if e5_3[-1]>e13_3[-1] else -0.25)
                scores["rsi_3m"]   = 0.5 if rsi3<45 else (-0.3 if rsi3>72 else 0.1)
            else:
                scores["cross_3m"] = 0.8 if cross_dn else (0.3 if e5_3[-1]<e13_3[-1] else -0.25)
        ...
        W = {
            ...
            "rsi_15m":1.5,"adx_15m":1.2,"ema_5m":1.6,"st_5m":1.3,
            "rsi_5m":1.1,"macd_5m":1.0,"bb":0.9,"volume":1.0,"vwap":0.7,
            "cross_3m":0.9,"rsi_3m":0.6,"liq":1.3,"funding":1.0,
            "ob":0.8,"ls":0.7,"oi":0.5,"learned":1.2,
        }
        ...
        # STRICT thresholds — research-backed
        thresholds  = {"CONSERVATIVE":0.45,"NORMAL":0.35,"AGGRESSIVE":0.28,"TURBO":0.25}
        conf_floors = {"CONSERVATIVE":72,  "NORMAL":65,  "AGGRESSIVE":58,  "TURBO":55}
