...
class QuantSignalEngine:
    ...
    def analyze(self, sym, k3m, k5m, k15m, k1h, k4h,
        ...
        if k4h is not None and len(k4h) >= 50:
            ...
            # ADX < 12 = dead flat, no directional trades (lowered from 18)
            if adx_4h_val < 12:
                hold["veto_reason"] = f"RANGING_ADX4H={adx_4h_val:.1f}<12"
                return hold
            ...
            elif adx_4h_val > 18:  # Lowered from 25 — use MACD to decide direction more often
                macro =  1 if hist4h > 0 else -1
            elif p4h > e50_4h[-1]:  # Weak bull — price above 50EMA even if EMAs aren't stacked
                macro = 1
            elif p4h < e50_4h[-1]:  # Weak bear
                macro = -1
            else:
            ...
            # Fallback: use 1H as macro
            c1h_tmp = k1h[:,4]
            e50_tmp = ema(c1h_tmp,50)
            adx_1h_tmp = calc_adx(k1h,14)
            if adx_1h_tmp < 12:  # Lowered from 20
                hold["veto_reason"] = f"RANGING_ADX1H={adx_1h_tmp:.1f}<12"
                return hold
        ...
        # ── GATE 2: 1H STRUCTURE ALIGNMENT (softened — partial agreement OK) ──
        h1_score = 0.0
        if k1h is not None and len(k1h) >= 50:
            ...
            if macro == 1:
                # Count how many 1H indicators agree with LONG
                agree = 0
                if p1h > e21_1h[-1]: agree += 1
                if st1h == 1: agree += 1
                if hist1h > 0: agree += 1

                if agree == 3:  # Full agreement
                    h1_score = 1.0 if (p1h > e50_1h[-1] and adx1h > 25) else 0.7
                elif agree == 2:  # Partial agreement — proceed with lower score
                    h1_score = 0.4
                else:  # 1H clearly disagrees — veto
                    hold["veto_reason"] = f"1H_DISAGREES_4H macro=LONG agree={agree}/3"
                    return hold
                ...
                if agree == 3:
                    ...
                    hold["veto_reason"] = f"1H_DISAGREES_4H macro=SHORT agree={agree}/3"
                    return hold
        elif k1h is not None and len(k1h) >= 20:
            # Minimal 1H data — use simple EMA direction
            c1h = k1h[:,4]
            e21_1h = ema(c1h, min(21, len(c1h)-1))
            p1h = float(c1h[-1])
            if (macro == 1 and p1h > e21_1h[-1]) or (macro == -1 and p1h < e21_1h[-1]):
                h1_score = 0.3
            else:
                hold["veto_reason"] = "1H_WEAK_DISAGREE"
                return hold
        else:
            # No 1H data — proceed with reduced confidence instead of vetoing
            h1_score = 0.2
            logger.debug("No 1H data — proceeding with reduced h1_score")
        ...
        if k5m is not None and len(k5m) >= 55:
            ...
            scores["st_5m"]  = 0.7 if st5==macro else -0.8  # Raised from -0.5: stronger veto when 5m disagrees
            scores["macd_5m"] = 0.6 if (hist5>0 and macro==1) or (hist5<0 and macro==-1) else -0.5  # Raised from -0.3
        ...
        if k3m is not None and len(k3m)>=30:
            ...
            if macro==1:
                scores["cross_3m"] = 0.8 if cross_up else (0.3 if e5_3[-1]>e13_3[-1] else -0.5)  # Raised from -0.25
                scores["rsi_3m"]   = 0.5 if rsi3<45 else (-0.3 if rsi3>72 else 0.1)
            else:
                scores["cross_3m"] = 0.8 if cross_dn else (0.3 if e5_3[-1]<e13_3[-1] else -0.5)  # Raised from -0.25
        ...
        if macro==1:
            scores["ob"] = 0.7 if ob_imb>2.0 else (0.35 if ob_imb>1.3 else (-0.7 if ob_imb<0.6 else 0.1))  # Raised penalty from -0.5→-0.7
        else:
            scores["ob"] = 0.7 if ob_imb<0.5 else (0.35 if ob_imb<0.77 else (-0.7 if ob_imb>1.7 else 0.1))  # Raised penalty from -0.5→-0.7
        ...
        # Weights — tuned from trade analysis:
        # ob raised 0.8→1.4 (weak OB present in 54% of losses)
        # st_5m raised 1.3→1.8 (5m SuperTrend disagreement = losses)
        # cross_3m raised 0.9→1.3 (3m cross negative in 33% of losses)
        # macd_5m raised 1.0→1.4 (5m MACD negative in 38% of losses)
        W = {
            "htf_alignment":3.0,"ema_15m":2.0,"macd_15m":1.8,"st_15m":1.7,
            "rsi_15m":1.5,"adx_15m":1.2,"ema_5m":1.6,"st_5m":1.8,
            "rsi_5m":1.1,"macd_5m":1.4,"bb":0.9,"volume":1.0,"vwap":0.7,
            "cross_3m":1.3,"rsi_3m":0.6,"liq":1.3,"funding":1.0,
            "ob":1.4,"ls":0.7,"oi":0.5,"learned":1.2,
        }
        ...
        # STRICT thresholds — tuned for continuous trading
        # Lowered to allow more entries while keeping quality filters upstream
        thresholds  = {"CONSERVATIVE":0.40,"NORMAL":0.32,"AGGRESSIVE":0.25,"TURBO":0.20}
        conf_floors = {"CONSERVATIVE":65,  "NORMAL":60,  "AGGRESSIVE":52,  "TURBO":48}
        thresh     = thresholds.get(mode,0.35)
        conf_floor = conf_floors.get(mode,65)
...
def sl_tp(side, entry, atr_val, k5m=None, k15m=None, sl_mult=2.0, tp_mult=3.5):
    """
    ATR SL/TP with structural awareness.
    SL: 2.0x ATR (raised from 1.2x — stops behind structure not noise)
    TP: 3.5x ATR (raised from 2.4x — minimum 1:1.75 R:R)
    """
...
# Backward-compatible alias so engine.py can import SignalEngine
SignalEngine = QuantSignalEngine
