...
async def dashboard():
    ...
    root_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dashboard.html")
    if os.path.exists(dash_path):
        return FileResponse(dash_path)
    if os.path.exists(root_path):
        return FileResponse(root_path)
    return JSONResponse({"error": "Dashboard not found. Place dashboard.html in static/ or project root."})
...
async def api_whale():
    return {
        ...
        "all_symbols":   engine.whales.get_all_symbols(),
        "cache_stats":   engine.whales.get_cache_stats(),
    }
