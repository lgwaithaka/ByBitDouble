...
def test_email() -> str:
    ...
    _ensure()
    result = engine.test_email()
    return json.dumps(result, indent=2)


@mcp.tool()
def send_daily_report() -> str:
    """Manually trigger the daily report email now."""
    _ensure()
    import asyncio
    ...
    if loop.is_running():
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            future = pool.submit(asyncio.run, engine.send_daily_report())
            future.result(timeout=15)
    else:
        asyncio.run(engine.send_daily_report())
    return json.dumps({"status": "Daily report email triggered. Check your inbox and Render logs."})


@mcp.tool()
def get_market_feed() -> str:
    """Get current market sentiment data including Fear & Greed Index."""
    _ensure()
    feed = engine.market_feed.to_dict()
    feed["confidence_long_adj"]  = engine.market_feed.confidence_adjustment("LONG")
    feed["confidence_short_adj"] = engine.market_feed.confidence_adjustment("SHORT")
    from engine import TARGET_MULTIPLIER
    feed["target_multiplier"] = TARGET_MULTIPLIER
    feed["target_pct"] = f"{int((TARGET_MULTIPLIER-1)*100)}%"
