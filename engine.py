...
class CompoundTradingEngine:
    ...
    async def scan_and_trade(self):
        ...
        # ── Liquidity Session Adjustment (continuous 24/7 trading, varying by liquidity) ──
        # High liquidity: London+NY overlap (12-16 UTC) + full NY session (13-21) + Asia open (00-04)
        ...
        if (12 <= _utc_h < 21) or (0 <= _utc_h < 4) or (21 <= _utc_h < 24):
            ...
            liq_session  = "MED"
            liq_conf_adj = 0    # standard thresholds
            liq_sym_adj  = -2   # scan fewer symbols in medium session
        else:
            liq_session  = "LOW"
            liq_conf_adj = +4   # require stronger signal in thin/quiet markets
            liq_sym_adj  = -4   # scan fewer symbols
        n_syms = max(4, n_syms + liq_sym_adj)
        sp("liquidity_session", liq_session)
        logger.debug(f"Liquidity session: {liq_session} (UTC {_utc_h:02d}:xx) | conf_adj={liq_conf_adj:+d} | n_syms={n_syms}")
