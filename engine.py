...
from db import (
    ...
    get_symbol_stats, all_symbol_stats, snap_hour, close_epoch_record,
    open_epoch_record, get_all_epochs
)
...
class CompoundTradingEngine:
    def __init__(self):
        ...
        self._partial_closes: Dict[str, bool] = {}  # Tracks which positions had 50% closed at 5%
        self._fg_cache: dict  = {}   # Fear & Greed index cache (refreshed hourly)
    ...
    async def fetch_data(self, symbol: str) -> Optional[Dict]:
        try:
            # Also fetch BTC 4H for macro gate (only if symbol is not BTC itself)
            btc_task = (self.client.klines("BTCUSDT","240",60)
                       if symbol!="BTCUSDT"
                       else self.client.klines(symbol,"240",60))
            ...
            results = await asyncio.gather(
                ...
                btc_task,                                 # BTC 4H — macro gate
                return_exceptions=True
            )
            k3,k5,k15,k1h,k4h,fr,lsr,oi,ob,liq,btc4h = results
            ...
            return {
                ...
                "btck4h": parse_klines(btc4h) if not isinstance(btc4h,Exception) else None,
    ...
    async def check_profit_targets(self):
        ...
        for pos in db_open:
            ...
            # Step 1: Close 50% at 5% profit → locks in gain, reduces risk
            # Step 2: Trail remaining 50% with tight stop, close fully at 10%+
            # This captures guaranteed profit WHILE preserving upside on runners

            partial_key = f"partial_{trade_id}"
            already_partial = self._partial_closes.get(partial_key, False)

            if unr_pct >= 10.0:
                # Full close at 10%+ — whether partial or not
                logger.info(f"TAKE PROFIT 10% | {sym} +{unr_pct:.2f}% | ${unr:.4f}")
                try:
                    close_qty = str(qty)  # Full remaining qty
                    resp = await self.client.close_pos(sym, side, close_qty)
                    if resp.get("retCode",-1) == 0:
                        ...
                        if partial_key in self._partial_closes:
                            del self._partial_closes[partial_key]
                        logger.info(f"FULL CLOSE 10% | {sym} | ${pnl:.4f}")
                except Exception as e:
                    logger.error(f"Full close error {sym}: {e}")
                continue

            elif unr_pct >= 5.0 and not already_partial and qty >= 0.002:
                # PARTIAL CLOSE: close 50% at 5% to lock in profit
                half_qty = round(qty * 0.50, 3)
                if half_qty > 0:
                    logger.info(f"PARTIAL CLOSE 50% at 5% | {sym} +{unr_pct:.2f}% | qty={half_qty}")
                    try:
                        resp = await self.client.close_pos(sym, side, str(half_qty))
                        if resp.get("retCode",-1) == 0:
                            self._partial_closes[partial_key] = True
                            # Move SL to breakeven + small buffer for remaining position
                            prec = price_precision(entry)
                            buf  = entry * 0.001
                            be   = round(entry + buf, prec) if side == "Buy" else round(entry - buf, prec)
                            try:
                                await self.client.set_tpsl(sym, sl=str(be))
                            except Exception:
                                pass
                            logger.info(f"PARTIAL BANKED | {sym} | SL moved to breakeven {be:.4f}")
                    except Exception as e:
                        logger.error(f"Partial close error {sym}: {e}")
                continue

            # ─ Trailing stop: activates at 7% (protect gains before 10% close)
            if unr_pct >= 7.0:
                try:
            ...
            # ─ Breakeven at 4% (give position room to breathe to 10%)
            elif unr_pct >= 4.0:
                try:
                    prec   = price_precision(entry)
                    buf    = entry * 0.0008
    ...
    async def execute(self, symbol: str, analysis: Dict, meta: Dict) -> bool:
        ...
        # Session-weighted sizing: +25% during peak liquidity 19:00-20:59 UTC
        from datetime import datetime as _dt, timezone as _tz
        _h = _dt.now(_tz.utc).hour
        if 19 <= _h <= 20:
            risk_usd = risk_usd * 1.25
            logger.debug(f"Session bonus +25% applied (hour={_h} UTC)")
    ...
    async def monitor(self):
        ...
        for pos in db_open:
            ...
            if sym in self.live_positions:
                ...
                # Adaptive cooldowns from weekly report analysis:
                # Win: 5s — re-enter fast on profitable symbols
                # Loss: 120s base + 60s per cumulative net losses on that symbol
                if win:
                    self.cooldown_until[sym] = time.time() + 5
                else:
    ...
    async def _pyramid_winners(self):
        ...
        for pos in db_open:
            ...
            min_profit_pct = 0.025   # 2.5% — confirmed breakout level, not noise
            if side == "Buy":
    ...
    async def _get_fear_greed(self) -> dict:
        """Fetch Alternative.me Fear & Greed Index (free, no API key required)."""
        now = time.time()
        if self._fg_cache.get("ts", 0) > now - 3600:  # Cache 1 hour
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=5)) as sess:
                async with sess.get("https://api.alternative.me/fng/?limit=1") as r:
                    if r.status == 200:
                        ...
                        self._fg_cache = {"value": val, "label": label, "ts": now}
                        sp("fear_greed_value", str(val))
                        sp("fear_greed_label", label)
                        logger.info(f"Fear & Greed: {val} ({label})")
                        return self._fg_cache
        except Exception as e:
            logger.debug(f"Fear & Greed API: {e}")
        return self._fg_cache or {"value": 50, "label": "Neutral"}

    @staticmethod
    def _in_trading_window() -> bool:
        """
        DEAD ZONE BLOCK: Only trade during 19:00-00:59 UTC.
        Dead zone 01:00-18:59 UTC is blocked — 0 wins, 17 losses in Week 1 data.
        """
        h = datetime.now(timezone.utc).hour
        return (h >= 19) or (h == 0)   # 19,20,21,22,23,0

    async def scan_and_trade(self):
        ce = self.compound

        # ── DEAD ZONE GATE — Hard block ──────────────────────────────────────
        if not self._in_trading_window():
            h = datetime.now(timezone.utc).hour
            logger.debug(f"Dead zone skip: hour={h} UTC (active window: 19:00-00:59 UTC)")
            return

        # ── Circuit breaker (bypass in AGGRESSIVE/TURBO) ──────────────────────
        mode = ce.state.mode
        if ce.check_circuit_breakers(
            ...
            if mode not in ("AGGRESSIVE","TURBO"):
                return
            logger.warning(f"CB active but bypassed in {mode} mode")
        ...
        fg_conf_adj = 0
        if fg_val > 80:
            fg_conf_adj = 10   # Extreme Greed: raise LONG conf floor
        elif fg_val < 20:
            fg_conf_adj = 8    # Extreme Fear:  raise SHORT conf floor

        # ── Auto-blacklist: symbols with 2+ trades and 0% win rate ────────────
        auto_blacklist = set()
        for row in all_symbol_stats():
            if (row.get("wins",0)+row.get("losses",0)) >= 2 and row.get("wins",0) == 0:
                auto_blacklist.add(row["symbol"])
        if auto_blacklist:
            logger.debug(f"Auto-blacklisted: {auto_blacklist}")
        ...
        for meta in top_syms:
            ...
            an   = self.signals.analyze(
                ...
                mode=mode,
                btck4h=data.get("btck4h"),        # BTC 4H for altcoin LONG gate
            )
