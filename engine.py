...
class CompoundTradingEngine:
    ...
    async def fetch_data(self, symbol: str) -> Optional[Dict]:
        try:
            ...
            # Guard: treat both Exception AND None as a failed response.
            # The Bybit liquidation endpoint (/v5/market/liquidation) returns
            # None when the symbol has no recent liquidations or the endpoint
            # is temporarily unavailable — not a Python Exception, so the old
            # `not isinstance(x, Exception)` check passed None straight into
            # the parser causing "NoneType has no attribute get".
            def _ok(v): return v is not None and not isinstance(v, Exception)

            k5p = parse_klines(k5) if _ok(k5) else None
            if k5p is not None:
                self._klines_cache[symbol] = k5p
            return {
                "k3m":     parse_klines(k3)  if _ok(k3)  else None,
                "k5m":     k5p,
                "k15m":    parse_klines(k15) if _ok(k15) else None,
                "k1h":     parse_klines(k1h) if _ok(k1h) else None,
                "k4h":     parse_klines(k4h) if _ok(k4h) else None,
                "funding": p_funding(fr)     if _ok(fr)  else 0.0,
                "ls":      p_ls(lsr)         if _ok(lsr) else 1.0,
                "oi_pct":  p_oi(oi)          if _ok(oi)  else 0.0,
                "ob_imb":  p_ob(ob)          if _ok(ob)  else 1.0,
                "liqs":    p_liq(liq)        if _ok(liq) else {},
            }
        except Exception as e:
