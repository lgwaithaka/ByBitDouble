...
class CompoundTradingEngine:
    ...
    async def check_profit_targets(self):
        """
        Progressive profit protection — let winners run, never exit early.

        SL ladder (moves above entry once in profit — never retreats):
          ≥ 2%  → SL to entry + 0.1%   guaranteed profit, room to breathe
          ≥ 5%  → SL to entry + 1.5%   lock in meaningful gain
          ≥ 10% → Trails at 1.5× ATR below current price
          ≥ 15% → Trails at 1.0× ATR   tighter as gains grow
          ≥ 20% → Trails at 0.7× ATR   squeezing the last drops

        TP ladder (only ever raised — never lowered):
          ≥ 5%  → TP lifted to current + 3× ATR
          ≥ 10% → TP lifted to current + 4× ATR
          ≥ 15% → TP lifted to current + 5× ATR

        Hard close only at 25%+ unrealised — maximum profit extraction.
        Positions below 2% profit are left completely alone.
        """
        ...
        for pos in db_open:
            ...
            # Leave flat / losing positions alone — SL/TP on exchange handles them
            if unr_pct < 2.0: continue

            # ── Fetch ATR once for all calculations ───────────────────────────
            atr_v = entry * 0.01  # safe fallback (1% of entry)
            try:
                ...
                logger.debug(f"ATR fetch {sym}: {e}")

            prec   = price_precision(cur)
            old_sl = float(pos.get("sl_price", 0) or 0)
            old_tp = float(pos.get("tp_price", 0) or 0)

            # ── HARD CLOSE at 25%+ — bank maximum profit ──────────────────────
            if unr_pct >= 25.0:
                logger.info(f"TAKE PROFIT 25% | {sym} +{unr_pct:.2f}% | ${unr:.4f}")
                try:
                    ...
                    if resp.get("retCode", -1) == 0:
                        ...
                        self.cooldown_until[sym] = time.time() + 15
                        logger.info(f"PROFIT BANKED 25% | {sym} | ${pnl:.4f}")
                except Exception as e:
                    logger.error(f"Profit close error {sym}: {e}")
                continue

            # ── SL LADDER — advances toward price, never retreats ─────────────
            if unr_pct >= 20.0:
                trail_mult = 0.7
                min_sl = round(cur - atr_v * trail_mult, prec) if side == "Buy" else round(cur + atr_v * trail_mult, prec)
            elif unr_pct >= 15.0:
                trail_mult = 1.0
                min_sl = round(cur - atr_v * trail_mult, prec) if side == "Buy" else round(cur + atr_v * trail_mult, prec)
            elif unr_pct >= 10.0:
                ...
                buf    = entry * 0.015
                min_sl = round(entry + buf, prec) if side == "Buy" else round(entry - buf, prec)
            else:
                # ≥ 2% — just above entry, tiny buffer guarantees no loss
                buf    = entry * 0.001
                min_sl = round(entry + buf, prec) if side == "Buy" else round(entry - buf, prec)

            # Only update SL if it strictly improves (never move stop backwards)
            sl_improved = (side == "Buy"  and min_sl > old_sl + atr_v * 0.05) or                           (side == "Sell" and (old_sl == 0 or min_sl < old_sl - atr_v * 0.05))

            # ── TP LADDER — ratchet upward only, never lowered ─────────────────
            if   unr_pct >= 15.0: tp_mult = 5.0
            elif unr_pct >= 10.0: tp_mult = 4.0
            elif unr_pct >=  5.0: tp_mult = 3.0
            else:                  tp_mult = 0.0

            new_tp = 0.0; tp_improved = False
            if tp_mult > 0:
                new_tp = round(cur + atr_v * tp_mult, prec) if side == "Buy" else round(cur - atr_v * tp_mult, prec)
                tp_improved = (side == "Buy"  and (old_tp == 0 or new_tp > old_tp + atr_v * 0.1)) or                               (side == "Sell" and (old_tp == 0 or new_tp < old_tp - atr_v * 0.1))

            # ── Apply SL / TP in one call ──────────────────────────────────────
            if sl_improved or tp_improved:
                try:
                    ...
                    parts = [f"+{unr_pct:.1f}%"]
                    if sl_improved: parts.append(f"sl={min_sl:.4f}")
                    if tp_improved: parts.append(f"tp={new_tp:.4f}")
                    logger.info(f"PROTECT | {sym} {' | '.join(parts)}")
                except Exception as e:
                    logger.debug(f"set_tpsl {sym}: {e}")

    # ── Diversification Gate ──────────────────────────────────────────────────

    def _can_open(self, symbol: str, signal: str) -> Tuple[bool, str]:
        """
        Gate check — simplified. Asset class restriction removed.
        We trade the best signals regardless of class overlap.
        """
        db_open   = get_open_positions()
        open_syms = {p["symbol"] for p in db_open}

        if symbol in open_syms:
            # Check if we should ADD to a winning position instead
            return False, "Already open (see pyramid logic)"

        # Only cooldown on losing symbols — winning symbols can re-enter
    ...
    async def monitor(self):
        ...
        for pos in db_open:
            ...
            if sym in self.live_positions:
                ...
                # Position absent from live feed — verify Bybit actually closed it.
                # A transient API gap, order still filling, or brief feed dropout
                # will show pnl=0 / exit_p=0.  Do NOT close the DB record in that
                # case — wait until Bybit returns a real avgExitPrice.
                pnl = 0.0; exit_p = 0.0
                try:
                ...
                if exit_p == 0:
                    # Bybit has no confirmed close — position still live on exchange.
                    # Skip this cycle; profit targets remain in force.
                    logger.debug(f"Position {sym} not in live feed but exit_price=0 — awaiting Bybit confirmation, skipping close")
                    continue
                ...
                if win:
                    ...
                    logger.debug(f"Adaptive cooldown {sym}: {cd_secs}s (net_losses={net_losses})")
                if not win: self.losing_symbols.add(sym)
                logger.info(f"CLOSED | {sym} | {outcome} | ${pnl:.4f}")

        # ── Pyramid: Add to profitable positions ──────────────────────────────
        await self._pyramid_winners()

    async def _pyramid_winners(self):
        """
        If a position is profitable by >1.0x ATR AND signal still valid:
        Add a second entry at 50% of original size.
        Raises effective position size on winning trades — maximises returns.
        Max 2 pyramid layers per position.
        """
        mode = self.compound.state.mode
        # Only pyramid in AGGRESSIVE or TURBO modes
        ...
        for pos in db_open:
            ...
            # Only pyramid if profitable by at least 0.8% of entry
            min_profit_pct = 0.008   # 0.8% in the money
            if side == "Buy":
                live_p = self.live_positions.get(sym, {})
                cur    = float(live_p.get("markPrice","0") or 0)
                if cur <= 0 or (cur - entry) / entry < min_profit_pct:
                    continue
            else:
                live_p = self.live_positions.get(sym, {})
                cur    = float(live_p.get("markPrice","0") or 0)
                if cur <= 0 or (entry - cur) / entry < min_profit_pct:
                    continue

            # Check we haven't already pyramided (tag contains PYR)
            tag = pos.get("tag") or ""
            if "PYR" in tag:
                continue  # Already added to this position

            # Add 50% of original size
            pyramid_qty = round(qty * 0.50, 3)
            min_notional = float(gp("min_notional_usdt","5.5"))
            if pyramid_qty * entry < min_notional:
                continue  # Too small

            # Place the add-on order (no new SL/TP — existing position's levels hold)
            try:
                ...
                if resp.get("retCode",0) == 0:
                    ...
                    # Mark in DB
                    open_trade(
                        epoch=ep, symbol=sym, side=side, signal=pos["signal"],
                        confidence=pos.get("confidence",50), composite=0.0,
                        entry_price=cur, qty=pyramid_qty,
    ...
    async def scan_and_trade(self):
        ...
        # ── Fear & Greed — fetch live, degrade gracefully on failure ──────────
            ...
            async with aiohttp.ClientSession() as _sess:
                async with _sess.get(
                    ...
                    if _r.status == 200:
                        ...
                        if   fg_val <= 20: fg_conf_adj = -3   # extreme fear  → buy dip
                        elif fg_val <= 35: fg_conf_adj = -1   # fear          → mild boost
                        elif fg_val >= 80: fg_conf_adj = +3   # extreme greed → cautious
                        elif fg_val >= 65: fg_conf_adj = +1   # greed         → mild caution
        except Exception as _fe:
            logger.debug(f"Fear & Greed unavailable: {_fe}")

        # ── Auto-blacklist: symbols with 0% win rate over ≥ 5 trades ─────────
        auto_blacklist: set = set()
        try:
            from db import get_all_symbol_stats
            for _ss in get_all_symbol_stats():
                if _ss.get("total", 0) >= 5 and _ss.get("wins", 0) == 0:
                    auto_blacklist.add(_ss["symbol"])
        except Exception:
            pass  # function may not exist yet — safe to continue without it

        # ── Symbol count — more in AGGRESSIVE/TURBO ───────────────────────────
        mode      = ce.state.mode
        n_syms    = int(gp("vol_scan_n","12"))
        if mode == "TURBO":        n_syms = min(n_syms + 4, 18)
        elif mode == "AGGRESSIVE": n_syms = min(n_syms + 2, 16)

        # ── 24/7 continuous trading — scan volume scales with liquidity ───────
        # Signal quality thresholds unchanged — only symbol count varies.
        #   High (00-04, 12-21 UTC): London + NY sessions + Asia open — full scan
        #   Med  (04-08, 21-24 UTC): Asia main + pre-London            — moderate
        #   Low  (08-12 UTC):        quiet crossover hours              — lean
        _h = datetime.now(timezone.utc).hour
        if (_h >= 12 and _h < 21) or (_h >= 0 and _h < 4):
            liq_session = "HIGH"
        elif (_h >= 4 and _h < 8) or (_h >= 21 and _h < 24):
            liq_session = "MED";  n_syms = max(4, n_syms - 2)
        else:
            liq_session = "LOW";  n_syms = max(4, n_syms - 4)
        sp("liquidity_session", liq_session)

        top_syms   = await self.scanner.scan(n=n_syms)
        adj        = self.regime.adjustments()
        # Confidence floor — never gated by time of day
        ...
        for meta in top_syms:
            ...
            if sym in auto_blacklist: continue  # 0% win rate — skip
            ...
            result = {
                ...
                "fear_greed": fg_val, "fg_label": fg.get("label","Neutral")
            }

            # LTF veto: if both 5m MACD and 3m cross are negative, skip
    ...
    async def main_loop(self):
        ...
        scan_secs = int(gp("scan_interval_s","30"))  # 30s — positions held until profit targets, never by time
        last_daily_hour = -1
