...
class CompoundTradingEngine:
    ...
    async def monitor(self):
        ...
        for pos in db_open:
            ...
            if sym in self.live_positions:
                ...
                if not win:
                    # Base cooldown: 120s, escalate for repeated losers
                    sym_stats = get_symbol_stats(sym)
                    consecutive = sym_stats.get("losses", 0) - sym_stats.get("wins", 0)
                    cd = min(600, 120 + (max(0, consecutive) * 60))  # Max 10 min cooldown
                    ...
                    # Remove from losing symbols on win
    ...
    async def scan_and_trade(self):
        ...
        # ── FILTER 1: Block losing hours (20:00-23:00 UTC = 0W/17L historically) ──
        current_hour = datetime.now(timezone.utc).hour
        blocked_hours = {20, 21, 22, 23}
        if current_hour in blocked_hours:
            logger.info(f"Skipping scan: hour {current_hour} UTC is in blocked window (20-23)")
            return

        # ── FILTER 2: Override CONSERVATIVE mode (2W/20L = 9% WR) ──
