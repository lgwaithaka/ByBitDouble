    ...
    async def monitor(self):
        ...
        for pos in db_open:
            ...
            if sym in self.live_positions:
                ...
                # Win cooldown: 5 seconds (re-enter fast if signal still valid)
                # Loss cooldown: 90 seconds (brief pause, then back in)
                self.cooldown_until[sym] = time.time() + (5 if win else 90)
