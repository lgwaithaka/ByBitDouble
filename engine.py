"""
engine.py v2.1 — Perpetual Compounding Trading Engine (Updated for Infinite Profit & Delayed Breakeven)
New features:
• DELAYED BREAKEVEN: Ignore trades until 4% profit to prevent 45s noise-outs.
• INFINITE PROFIT: No hard cap. Trades exit ONLY via trailing stop.
• TRAILING STOP LADDER: 
    - ≥ 4%  → SL to Entry + 1% Buffer
    - ≥ 10% → Trail at 1.5x ATR
    - ≥ 20% → Trail at 0.7x ATR (Tight squeeze)
"""
...
class CompoundTradingEngine:
    ...
    # ── 4% Profit Taking + Trailing Stop (Infinite Upside) ────────────────────

    async def check_profit_targets(self):
        """
        Progressive profit protection — let winners run indefinitely.

        Changes applied:
        • DELAYED BREAKEVEN: Do not move SL until >= 4.0% profit (prevents 45s noise-outs).
        • INFINITE PROFIT: Removed hard 25% cap. Trades exit ONLY via trailing stop.
        """
        ...
        for pos in db_open:
            ...
            # 🚀 FIX 1: DELAYED INTERVENTION
            # Leave positions alone until they secure at least 4% profit.
            # This prevents the "45s breakeven" loop caused by market noise.
            if unr_pct < 4.0:
                continue
            ...
            # 🚀 FIX 2: INFINITE PROFIT (Removed 25% Hard Cap)
            # We no longer hard-close at 25%. Winners run until trailing stop catches them.

            # ── SL LADDER — advances toward price ─────────────
            if side == "Buy":
                if   unr_pct >= 20.0: min_sl = round(cur - atr_v * 0.7, prec) # Tight Trail
                elif unr_pct >= 10.0: min_sl = round(cur - atr_v * 1.5, prec) # Loose Trail
                else:                 min_sl = round(entry * 1.01, prec)      # Breakeven + 1% (at >= 4%)
            else:
                if   unr_pct >= 20.0: min_sl = round(cur + atr_v * 0.7, prec)
                elif unr_pct >= 10.0: min_sl = round(cur + atr_v * 1.5, prec)
                else:                 min_sl = round(entry * 0.99, prec)

            sl_improved = (side == "Buy"  and (old_sl == 0 or min_sl > old_sl + atr_v * 0.05)) or \
                          (side == "Sell" and (old_sl == 0 or min_sl < old_sl - atr_v * 0.05))

            # ── TP LADDER (Raises target to encourage holding, but SL is the exit) ──
            new_tp = 0.0; tp_improved = False
            if   unr_pct >= 20.0: tp_mult = 10.0 # Push TP far away
            elif unr_pct >= 10.0: tp_mult = 7.0
            else:                 tp_mult = 5.0
            
            if tp_mult > 0:
                new_tp = round(cur + atr_v * tp_mult, prec) if side == "Buy" else round(cur - atr_v * tp_mult, prec)
                tp_improved = (side == "Buy"  and (old_tp == 0 or new_tp > old_tp)) or \
    ...
    async def main_loop(self):
        ...
        scan_secs = int(gp("scan_interval_s", "60"))  # 60s — Continuous scanning
        last_daily_hour = -1
