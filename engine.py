...
Fixed: Liquidation API None handling, improved error resilience
"""
...
class CompoundTradingEngine:
    ...
    async def fetch_data(self, symbol: str) -> Optional[Dict]:
        """Fetch all market data for a symbol with robust error handling."""
        try:
            ...
            # ✅ FIX: Handle liquidation data safely - can be None
            liq_data = {}
            if not isinstance(liq, Exception) and liq is not None:
                try:
                    liq_data = p_liq(liq) or {}
                except Exception as e:
    ...
    async def execute(self, symbol: str, analysis: Dict, meta: Dict) -> bool:
        ...
        open_trade(epoch=epoch, symbol=symbol, side=side, signal=signal, confidence=conf, composite=comp,
            ...
            components=analysis.get("components",{}), vol_score=meta.get("vol_score",0),
    ...
    async def scan_and_trade(self):
        ce = self.compound
        if ce.check_circuit_breakers(self.balance, epoch_max_dd=float(gp("epoch_max_dd_pct", "35"))/100,
        ...
        try:
            ...
            async with aiohttp.ClientSession() as _sess:
                async with _sess.get("https://api.alternative.me/fng/?limit=1", timeout=aiohttp.ClientTimeout(total=5)) as _r:
                    if _r.status == 200:
                        _fng = await _r.json()
                        fg = _fng.get("data", [{}])[0]
