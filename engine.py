...
class CompoundTradingEngine:
    ...
    async def check_profit_targets(self):
        ...
        for pos in db_open:
            ...
            # ─ Take profit at 10% (hold until profitable) ────────────────
            # Per strategy: hold position until 10% profit before closing
            if unr_pct >= 10.0:
    ...
    async def scan_and_trade(self):
        ...
        # ── Fear & Greed — fetch live, fall back gracefully ───────────────────
        ...
        try:
            ...
            async with aiohttp.ClientSession() as _sess:
                async with _sess.get(
                    ...
                    if _r.status == 200:
                        ...
                        # Adjust confidence floor: extreme fear = easier LONG entry,
                        # extreme greed = harder LONG, easier SHORT
            ...
            logger.debug(f"Fear & Greed fetch skipped: {_e}")

        # ── Auto-blacklist: symbols with 0% win rate (≥5 trades) ─────────────
        ...
        try:
            ...
            pass  # DB function may not exist yet — safe to skip

        # ── Symbol count — AGGRESSIVE/TURBO scan more ─────────────────────────
        mode      = ce.state.mode
        n_syms    = int(gp("vol_scan_n","12"))
        if mode == "TURBO":        n_syms = min(n_syms + 4, 18)
        elif mode == "AGGRESSIVE": n_syms = min(n_syms + 2, 16)

        # ── 24/7 continuous trading — volume scales with market liquidity ─────
        # Engine always trades. Position sizing, risk management, and arbitrage
        # do the heavy lifting. We scan more symbols during deep-market hours
        # and fewer during thin hours to maximise fill quality and avoid slippage.
        #   High (00-04, 12-21 UTC): London + NY sessions, Asia open — full scan
        #   Med  (04-08, 21-24 UTC): Asia main + pre-London            — moderate
        #   Low  (08-12 UTC):        quiet crossover                    — lean
        ...
        # Confidence floor — signal quality never gated by time of day
