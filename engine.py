...
class CompoundTradingEngine:
    ...
    async def check_profit_targets(self):
        ...
        SL ladder (moves above entry once in profit — never back down):
          ≥ 2%  → SL to entry + 0.1%  (guaranteed profit, position breathes)
          ≥ 5%  → SL to entry + 1.5%  (lock in meaningful gain)
          ≥ 10% → SL trails at 1.5×ATR below current price
          ≥ 15% → SL trails at 1.0×ATR (tighter — protect larger gain)
          ≥ 20% → SL trails at 0.7×ATR (very tight — squeeze last drops)

        TP ladder (ratchets UP as profit grows — never lowered):
        ...
        for pos in db_open:
            ...
            # Skip losing / flat positions — only manage profitable ones
            if unr_pct < 2.0: continue

            # ── Fetch ATR once for all calculations ───────────────────────
            atr_v = entry * 0.01  # safe fallback
            try:
            ...
            # ── HARD CLOSE at 25%+ — maximum profit, don't give it back ──
            if unr_pct >= 25.0:
            ...
            # ── SL LADDER — moves above entry, never retreats ─────────────
            # Determine the minimum SL we should have right now
            if unr_pct >= 20.0:
                # Very tight trail — squeeze every last drop
                trail_mult = 0.7
                if side == "Buy":
                    min_sl = round(cur - atr_v * trail_mult, prec)
                else:
                    min_sl = round(cur + atr_v * trail_mult, prec)
            elif unr_pct >= 15.0:
                # Tight trail
                ...
                # Standard trail — let position breathe
                trail_mult = 1.5
                if side == "Buy":
                    min_sl = round(cur - atr_v * trail_mult, prec)
                else:
                ...
                # Lock in 1.5% above entry — meaningful profit guaranteed
                buf = entry * 0.015
                if side == "Buy":
                    min_sl = round(entry + buf, prec)
                else:
                    min_sl = round(entry - buf, prec)
            else:
                # ≥2% — just above entry, tiny buffer so we never lose money
                buf = entry * 0.001
                if side == "Buy":
                    min_sl = round(entry + buf, prec)
                else:
            ...
            # Only move SL if it improves (never retreat a stop)
            sl_improved = False
            if side == "Buy"  and min_sl > old_sl + (atr_v * 0.05):
                sl_improved = True
            elif side == "Sell" and (old_sl == 0 or min_sl < old_sl - (atr_v * 0.05)):
                sl_improved = True

            # ── TP LADDER — ratchet upward as profit grows ─────────────────
            # TP is only ever raised, never lowered — let the position run
            if unr_pct >= 15.0:
                tp_mult = 5.0
            elif unr_pct >= 10.0:
                tp_mult = 4.0
                ...
                tp_mult = 0.0  # below 5% — don't touch TP yet

            new_tp = 0.0
            tp_improved = False
            if tp_mult > 0:
                if side == "Buy":
                    new_tp = round(cur + atr_v * tp_mult, prec)
                    if old_tp == 0 or new_tp > old_tp + (atr_v * 0.1):
                        tp_improved = True
                else:
                    new_tp = round(cur - atr_v * tp_mult, prec)
                    if old_tp == 0 or new_tp < old_tp - (atr_v * 0.1):
            ...
            # ── Apply SL and/or TP update in one API call ──────────────────
            if sl_improved or tp_improved:
                try:
                    ...
                    log_parts = [f"+{unr_pct:.1f}%"]
                    if sl_improved: log_parts.append(f"sl={min_sl:.4f}")
                    if tp_improved: log_parts.append(f"tp={new_tp:.4f}")
                    logger.info(f"PROTECT | {sym} {' | '.join(log_parts)}")
                except Exception as e:
    ...
    async def scan_and_trade(self):
        ...
        # Scan more symbols in AGGRESSIVE/TURBO for more opportunities
        mode      = ce.state.mode
