...
class CompoundTradingEngine:
    def __init__(self, client=None, symbols=None):
        ...
        Args:
            client: BybitClient instance (optional, can be set later via init())
            symbols: List of symbols to trade (optional)
        """
        self.client: Optional[BybitClient] = client
        self.scanner: Optional[VScanner] = VScanner(client) if client else None
    ...
    def init(self, api_key: str, api_secret: str, testnet: bool = False):
        """Initialize with API credentials if not already set."""
        if not self.client:
    ...
    async def check_profit_targets(self):
        ...
        for pos in db_open:
            ...
            sl_improved = (side == "Buy"  and min_sl > old_sl + atr_v * 0.05) or \
            ...
            if tp_mult > 0:
                ...
                tp_improved = (side == "Buy"  and (old_tp == 0 or new_tp > old_tp + atr_v * 0.1)) or \
                              (side == "Sell" and (old_tp == 0 or new_tp < old_tp - atr_v * 0.1))
