...
class WhaleTracker:
    ...
    async def _analyze_symbol(self, sym: str, client: BybitClient) -> Dict:
        ...
        funding_rate = 0.0
        fr_list = fr_resp.get("result", {}).get("list", []) or []
        if fr_list:
            funding_rate = float(fr_list[0].get("fundingRate", "0") or "0")

        oi_change = 0.0
        oi_list = oi_resp.get("result", {}).get("list", []) or []
        if len(oi_list) >= 2:
...
class CompoundTradingEngine:
    ...
    async def fetch_data(self, symbol: str) -> Optional[Dict]:
        def _safe(fn, arg, default):
            """Safe parser wrapper — catches NoneType and any other errors."""
            try:
                if isinstance(arg, Exception): return default
                return fn(arg)
            except Exception as e:
                logger.debug(f"parse error {fn.__name__} for {symbol}: {e}")
                return default
        ...
        try:
            ...
            return {
                "k3m":     _safe(parse_klines, k3,  None),
                "k5m":     _safe(parse_klines, k5,  None),
                "k15m":    _safe(parse_klines, k15, None),
                "k1h":     _safe(parse_klines, k1h, None),
                "k4h":     _safe(parse_klines, k4h, None),
                "funding": _safe(p_funding, fr,  0.0),
                "ls":      _safe(p_ls,      lsr, 1.0),
                "oi_pct":  _safe(p_oi,      oi,  0.0),
                "ob_imb":  _safe(p_ob,      ob,  1.0),
                "liqs":    _safe(p_liq,     liq, {}),
            }
        except Exception as e:
