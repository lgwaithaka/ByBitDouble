...
def vwap(klines: np.ndarray) -> float:
    """
    Calculate Volume Weighted Average Price.
    Args: klines - numpy array with columns [ts, open, high, low, close, volume]
    Returns: VWAP value
    """
...
def p_liq(liq_data) -> Dict:
    ...
    """
    try:
        # Handle None, empty dict, or non-dict
        if liq_data is None:
            return {"long_vol": 0, "short_vol": 0, "net": 0}
        
        if not isinstance(liq_data, dict):
            logger.debug(f"p_liq: unexpected type {type(liq_data)}")
            return {"long_vol": 0, "short_vol": 0, "net": 0}
