    ...
    async def scan(self, n: int = 12, force: bool = False) -> List[Dict]:
        ...
        for sym in self.symbols_pool:
            try:
                ...
                if data:  # ✅ FIX: Added missing condition
                    results.append(data)
...
def vwap(klines: np.ndarray, period: int = None) -> float:
    ...
    Args: 
        klines: numpy array with columns [ts, open, high, low, close, volume]
        period: optional, if provided only use last N periods
    Returns: VWAP value
    """
    ...
    vwap_val = np.sum(tpv) / np.sum(volumes) if np.sum(volumes) > 0 else 0.0
...
def p_liq(liq_data) -> Dict:
    """
    Parse liquidation data - handles None gracefully.
    """
    try:
        if liq_data is None:
        ...
        logger.debug(f"p_liq unexpected error: {e}")
        return {"long_vol": 0, "short_vol": 0, "net": 0}
...
def macd(closes: np.ndarray, fast: int = 12, slow: int = 26, signal: int = 9) -> Tuple[float, float, float]:
    ...
    signal_line = ema(macd_line[-signal*2:], signal)
